...
var Potrace = (function() {
  ...
  function processPaths(paths, params) {
    ...
    function calcPath(path) {
      ...
      path.curve = curve = new Curve(n);
      
      for (i = 0; i < n; i++) {
        j = mod(i+1, n);
        curve.vertex[i] = new Point(pt[j].x - pt[i].x, pt[j].y - pt[i].y);
      }

      var sum = 0;
      for (i = 0; i < n; i++) {
        sum += xprod(pt[i], pt[mod(i+1, n)]);
      }
      ...
      for (i = 0; i < n; i++) {
        ...
        lon[i] = m.sqrt(m.pow(p0.x-p2.x,2)+m.pow(p0.y-p2.y,2));
      }
      ...
      for (i=0; i<n; i++) {
        ...
        q = new Point(p2.x-p1.x, p2.y-p1.y);
        
        d = m.pow(q.x,2)+m.pow(q.y,2);
        
        if (d > 0.0) {
          curve.c[i][2] = p1;
          curve.c[i][1] = new Point(p1.x+q.x/3.0, p1.y+q.y/3.0);
          curve.c[mod(i-1,n)][0] = new Point(p1.x-q.x/3.0, p1.y-q.y/3.0);
        }
      }
      curve.c[n-1][0] = curve.c[0][0];
