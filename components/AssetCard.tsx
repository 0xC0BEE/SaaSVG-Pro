        ...
        <div className="flex items-center gap-2">
            {asset.png && asset.svg && (
                 <div className="flex items-center space-x-1 bg-[#0A0A0A] p-1 rounded-md border border-gray-700">
                    <button onClick={() => setViewMode('svg')} className={`px-2 py-1 rounded flex items-center gap-1 ${viewMode === 'svg' ? 'bg-[#00D4AA] text-black' : 'text-gray-400 hover:bg-gray-700'}`}><VectorIcon /> SVG</button>
                    <button onClick={() => setViewMode('png')} className={`px-2 py-1 rounded flex items-center gap-1 ${viewMode === 'png' ? 'bg-[#00D4AA] text-black' : 'text-gray-400 hover:bg-gray-700'}`}><ImageIcon /> PNG</button>
                </div>
            )}
        </div>
        <div className="flex items-center gap-2">
